...
func registerMCPTools(server *mcp.Server) {
	...
	// Register analyze_agent_roster tool
	mcp.AddTool(server, &mcp.Tool{
		Name: "analyze_agent_roster",
		Description: "Analyze tech stack from STRATEGIES.yaml and identify gaps in available agents. " +
			"Use when user asks 'what agents should I create?' or 'help me set up agents'. " +
			"Returns tech stack components, available agents that cover each component, " +
			"gap analysis (missing specialists), and recommendations for agent creation.",
	}, func(ctx context.Context, req *mcp.CallToolRequest, args struct {
		SourceName string `json:"source_name,omitempty"` // Optional - analyze specific source
	}) (*mcp.CallToolResult, any, error) {
		// Determine which source to analyze
		sourceName := args.SourceName
		if sourceName == "" {
			sourceName = "my-agents" // Default to my-agents
		}
		...
		// Find source
		source, err := cfg.GetAgentSource(sourceName)
		if err != nil {
			return nil, nil, fmt.Errorf("source not found: %w", err)
		}

		// Read STRATEGIES.yaml from source
		strategiesPath := filepath.Join(source.Path, "STRATEGIES.yaml")
		strategiesData, err := os.ReadFile(strategiesPath)
		if err != nil {
			return nil, nil, fmt.Errorf("STRATEGIES.yaml not found in source '%s': %w\nPath: %s", sourceName, err, strategiesPath)
		}

		// Parse STRATEGIES.yaml
		...
		if err := yaml.Unmarshal(strategiesData, &strategies); err != nil {
			return nil, nil, fmt.Errorf("failed to parse STRATEGIES.yaml: %w", err)
		}

		// Load all available agents from this source
		agentSources := []agent.AgentSource{
			{
				Path:     source.Path,
				Priority: source.Priority,
			},
		...
		allAgents, err := agent.LoadAgentsFromSources(agentSources)
		if err != nil {
			return nil, nil, fmt.Errorf("failed to load agents from source: %w", err)
		}

		// Analyze tech stack components from preference
		techComponents := analyzeTechStackComponents(strategies.TechStack.Preference)

		// Match agents to components based on name/description
		componentCoverage := make(map[string][]string)
		for _, component := range techComponents {
			for _, ag := range allAgents {
				if agentCoversComponent(ag, component) {
					componentCoverage[component] = append(componentCoverage[component], ag.Name)
				}
			}
		}
		...
		var covered []string
		for _, component := range techComponents {
			if len(componentCoverage[component]) == 0 {
				gaps = append(gaps, component)
			} else {
		...
		// Format response
		responseText := fmt.Sprintf("# Agent Roster Analysis: %s\n\n", sourceName)
		responseText += "## Tech Stack (from STRATEGIES.yaml)\n\n"
		responseText += fmt.Sprintf("**Preference**: %s\n\n", strategies.TechStack.Preference)

		responseText += "## Component Coverage\n\n"
		for _, component := range techComponents {
			agents := componentCoverage[component]
			if len(agents) > 0 {
				responseText += fmt.Sprintf("✓ **%s**: Covered by %s\n", component, strings.Join(agents, ", "))
			} else {
				responseText += fmt.Sprintf("✗ **%s**: No agent\n", component)
			}
		}

		if len(gaps) > 0 {
			responseText += "\n## Gaps Identified\n\n"
			responseText += fmt.Sprintf("Missing %d specialist agents:\n\n", len(gaps))
			for i, gap := range gaps {
				responseText += fmt.Sprintf("%d. **%s** - No specialist agent available\n", i+1, gap)
			}

			responseText += "\n## Recommendations\n\n"
			responseText += "Create the following agents using agent-architect:\n\n"
			for i, gap := range gaps {
				responseText += fmt.Sprintf("%d. **%s agent** - Specialist for %s\n", i+1, strings.ToLower(gap), gap)
			}
			responseText += "\nExample: \"Create a frontend agent for this source\"\n"
		} else {
			responseText += "\n## Status\n\n"
			responseText += "✓ **All tech stack components covered!**\n\n"
			responseText += "Your agent roster has specialists for all identified components.\n"
		}
		...
		return &mcp.CallToolResult{
			...
			"source":        sourceName,
			"tech_stack":    strategies.TechStack.Preference,
...
// ========== HELPER FUNCTIONS FOR AGENT ROSTER ANALYSIS ==========

// analyzeTechStackComponents parses a tech stack preference and returns a list of components.
// For example, "modern-web" returns ["Frontend", "Backend", "Database", "Testing", "Infrastructure"]
func analyzeTechStackComponents(preference string) []string {
	// Normalize preference to lowercase for matching
	pref := strings.ToLower(preference)

	// Define component mappings for common tech stack preferences
	techStackMappings := map[string][]string{
		"modern-web": {
			"Frontend",
			"Backend",
			"Database",
	...
	if components, exists := techStackMappings[pref]; exists {
		return components
	}
...
func agentCoversComponent(ag *agent.Agent, component string) bool {
	// Normalize for case-insensitive matching
	componentLower := strings.ToLower(component)
	nameLower := strings.ToLower(ag.Name)
	descLower := strings.ToLower(ag.Description)

	// Direct name match (e.g., "frontend" agent covers "Frontend" component)
	if strings.Contains(nameLower, componentLower) {
		return true
	}

	// Check description for component keywords
	if strings.Contains(descLower, componentLower) {
		return true
	}

	// Component-specific keyword matching
	componentKeywords := map[string][]string{
		"frontend": {"ui", "interface", "react", "vue", "angular", "client", "browser"},
		"backend":  {"server", "api", "service", "endpoint"},
		"database": {"db", "sql", "postgres", "mysql", "mongo", "data store"},
		"testing":  {"test", "qa", "quality", "spec", "e2e", "integration"},
		"infrastructure": {"deploy", "docker", "kubernetes", "ci/cd", "devops", "infra"},
		"api":      {"rest", "graphql", "endpoint", "route"},
		"security": {"auth", "authentication", "authorization", "security", "crypto"},
		"ios":      {"swift", "ios", "apple", "xcode"},
		"android":  {"kotlin", "android", "gradle"},
		"data processing": {"etl", "pipeline", "batch", "stream"},
		"machine learning": {"ml", "model", "training", "prediction", "ai"},
		"visualization": {"chart", "graph", "dashboard", "viz"},
		"devops":   {"deploy", "ci", "cd", "pipeline", "automation"},
		"observability": {"monitoring", "logging", "tracing", "metrics"},
	}

	// Check if agent name or description contains component keywords
	keywords, exists := componentKeywords[componentLower]
	if exists {
